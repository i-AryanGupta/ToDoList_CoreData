...
class ViewController: UIViewController, UITableViewDelegate, UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = UITableViewCell(style: .subtitle, reuseIdentifier: "cell")
            ...
            // Configure the task name in the main textLabel
            ...
            // Configure the date in the detailTextLabel and adjust the font size
            if let createdAt = model.createdAt {
                let formattedDate = formatter.string(from: createdAt)
                cell.detailTextLabel?.text = formattedDate
                cell.detailTextLabel?.font = UIFont.systemFont(ofSize: 12) // smaller font for date
                cell.detailTextLabel?.textColor = .gray // optional: change the color of the date text to gray
            }
            return cell
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        sheet.addAction(UIAlertAction(title: "Edit", style: .default, handler: { _ in
            ...
            alert.addTextField(configurationHandler: nil)
            alert.textFields?.first?.text = item.name // text will be pre-filled
